...
Item {
    ...
    function launch(exec_, terminal_, appName_) {
        ...
        // setsid + disown fully detaches the launched app from this shell
        // so a non-forking / slow-to-daemonize app can never block future launches.
        var detachedCmd = "setsid -f sh -c " + "'" + cmd.replace(/'/g, "'\\''") + "'" + " >/dev/null 2>&1 & disown";
        var proc = launchProcComponent.createObject(root, {
            command: ["sh", "-c", detachedCmd]
        });
        proc.running = true;
                ...
                RowLayout {
                    ...
                    Item {
                        ...
                        Text {
                            ...
                            text: row.starred ? "★" : "☆"
                            font.pixelSize: 14
                            color: row.starred ? theme.clrStar : starMa.containsMouse ? theme.clrStar : (row.mouseOver || row.isSel) ? theme.clrStarOff : "transparent"
                            Behavior on color {
                                ColorAnimation {
                                    duration: 100
                                }
